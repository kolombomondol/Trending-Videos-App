...
def extract_items_and_cursor(data):
    """
    tikwm API-র response গঠন কখনো কখনো ভিন্ন হতে পারে,
    তাই সম্ভাব্য সবগুলো ফরম্যাট চেক করা হচ্ছে।
    """
    ...
    # কেস ১: data সরাসরি list
    if isinstance(payload, list):
        return payload, None, False

    # কেস ২: data একটা dict, ভিতরে videos/list key
    if isinstance(payload, dict):
        items = payload.get("videos") or payload.get("list") or []
...
def fetch_worldwide_viral_videos():
    ...
    for code, country_name in countries.items():
        ...
        added_ids = set()
        cursor = 0
        max_attempts = 5  # সর্বোচ্চ ৫ বার রিকোয়েস্ট করে ৫০টা পাওয়ার চেষ্টা করবে
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                if response.status_code != 200:
                    print(f"  ⚠️ Status {response.status_code} for {country_name}, stopping this country")
                    break

                data = response.json()
                items, next_cursor, has_more = extract_items_and_cursor(data)

                if attempt == 0 and not items:
                    # প্রথম রিকোয়েস্টেই কিছু না পেলে response টা প্রিন্ট করে দেখাও (ডিবাগের জন্য)
                    print(f"  ⚠️ No items found for {country_name}. Raw response keys: {list(data.keys())}")

                if not items:
                    break  # আর ডেটা নেই, লুপ থামাও
                ...
                for item in items:
                    ...
                    video_obj = {
                        ...
                        if isinstance(item.get("author"), dict) else item.get("author", "TikTok Creator"),
                        "playUrl": play_url,
                ...
                # পরের পেজের জন্য cursor আপডেট
                if next_cursor is not None:
                    cursor = next_cursor
                else:
                    cursor += len(items)  # cursor field না থাকলে fallback হিসেবে item সংখ্যা যোগ করা

                if not has_more and next_cursor is not None and len(items) < params["count"]:
                    break  # API নিজেই বলছে আর ডেটা নেই

            except Exception as e:
                print(f"⏩ Skipped {country_name} attempt {attempt+1} due to: {e}")
                break
        ...
        if country_videos:
            ...
            print(f"❌ 0 videos saved for {country_name}")
